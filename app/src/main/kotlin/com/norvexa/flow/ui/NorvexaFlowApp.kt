    ...
    if (showAddSheet) {
        ModalBottomSheet(
            ...
            Column(Modifier.padding(bottom = 24.dp)) {
                ...
                AddSheetItem(Icons.Rounded.Payments, "Доход") {
                    dialog = DialogType.INCOME
                    showAddSheet = false
                }
                AddSheetItem(Icons.Rounded.ReceiptLong, "Расход") {
                    dialog = DialogType.EXPENSE
                    showAddSheet = false
                }
                AddSheetItem(Icons.Rounded.AccountBalanceWallet, "Кошелёк") {
                    dialog = DialogType.WALLET
                    showAddSheet = false
                }
                AddSheetItem(Icons.Rounded.PersonAdd, "Клиент") {
                    dialog = DialogType.CLIENT
                    showAddSheet = false
                }
                AddSheetItem(Icons.Rounded.Payments, "Ожидаемая оплата") {
                    dialog = DialogType.RECEIVABLE
                    showAddSheet = false
                }
                AddSheetItem(Icons.Rounded.CalendarMonth, "Будущий расход") {
                    dialog = DialogType.PLANNED_EXPENSE
                    showAddSheet = false
                }
                AddSheetItem(Icons.Rounded.Savings, "Финансовый резерв") {
